...
final class LocalAPIServerTests: XCTestCase {
    func testHealthEndpointReportsLoopbackAndSDKReadiness() async throws {
        ...
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness())
        ...
        XCTAssertEqual(object["service"] as? String, api2agentBrand.displayName)
        ...
        XCTAssertEqual(object["missing"] as? [String], ["api2agentKey"])
    ...
    func testHealthEndpointReportsReadyWhenInlineKeyIsPresent() async throws {
        let port = try unusedTCPPort()
        let settings = api2agentSettings(port: port, api2agentKey: "crsr_test")
        let server = LocalAPIServer(settingsProvider: { settings }, harness: MockHarness())
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
    ...
    func testHealthEndpointReportsNeedsUnlockForSavedKeyOnly() async throws {
        ...
        let settings = api2agentSettings(
            port: port,
            keychainapi2agentKeyAvailable: true,
            api2agentBaseURL: "https://exchange.example",
            backendBaseURL: "https://redacted-backend.example",
            localAgentEndpoint: "/redacted/sdk/run"
        )
    ...
    func testRootEndpointReportsLocalOpenAICompatibilityMetadata() async throws {
        ...
        for path in ["/", "/v1"] {
            ...
            XCTAssertEqual(object["service"] as? String, api2agentBrand.displayName, path)
    ...
    func testRequestObserverRecordsJSONStreamingAndErrorRequests() async throws {
        ...
        let server = LocalAPIServer(
            settingsProvider: { api2agentSettings(port: port) },
            harness: MockHarness(),
    ...
    func testStartFailsWhenPortIsAlreadyInUse() throws {
        let port = try unusedTCPPort()
        let first = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness())
        let second = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness())
    ...
    func testStartFallsBackWhenPreferredPortIsAlreadyInUse() async throws {
        ...
        let first = LocalAPIServer(settingsProvider: { api2agentSettings(port: preferredPort) }, harness: MockHarness())
        let second = LocalAPIServer(settingsProvider: { api2agentSettings(port: preferredPort) }, harness: MockHarness())
    ...
    func testModelRetrieveEndpointAcceptsProviderPrefixedAliases() async throws {
        ...
        let paths = [
            "/v1/models/api2agent%2Fcomposer-2.5-fast-sdk",
            "/v1/models/api2agent/composer-2.5-fast-sdk"
        ]
    ...
    func testRequestModelAliasesNormalizeToComposerModels() async throws {
        ...
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness(recorder: recorder))
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
        ...
        request.httpMethod = "POST"
        request.httpBody = Data(#"{"model":"api2agent/composer-2.5-fast-sdk","input":"hello"}"#.utf8)
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        let (data, response) = try await URLSession.shared.data(for: request)
        XCTAssertEqual((response as? HTTPURLResponse)?.statusCode, 200)
        let object = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
    ...
    func testCORSPreflightAllowsSessionHeaders() async throws {
        ...
        XCTAssertTrue(allowedHeaders.contains("X-api2agent-Session"))
    ...
    func testCompletionsStreamingEndpoint() async throws {
        let port = try unusedTCPPort()
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness(events: [
            .text("hel"),
            .text("lo"),
    ...
    func testResponsesInputTokensEndpointReturnsLocalEstimate() async throws {
        ...
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: harness)
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
    ...
    func testResponsesStateIsBoundedAndVisibleInHealth() async throws {
        let port = try unusedTCPPort()
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness(), responseStateLimit: 2)
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
    ...
    func testResponsesConcurrentProjectMetadataKeepsIndependentSessions() async throws {
        let port = try unusedTCPPort()
        let recorder = PreparedRequestRecorder()
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness(delayNanoseconds: 50_000_000, recorder: recorder))
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
    ...
    func testChatCompletionsShortCircuitsOpenCodeTitleGeneration() async throws {
        let port = try unusedTCPPort()
        let recorder = PreparedRequestRecorder()
        let server = LocalAPIServer(settingsProvider: { api2agentSettings(port: port) }, harness: MockHarness(text: "sdk title", recorder: recorder))
        try server.start(port: port)
        defer { server.stop() }
        try await Task.sleep(nanoseconds: 150_000_000)
...
private func sendResponseRequest(port: UInt16, body: String) async throws {
    ...
    guard status == 200 else {
        throw NSError(domain: "api2agentTests", code: status ?? -1)
    }
}
