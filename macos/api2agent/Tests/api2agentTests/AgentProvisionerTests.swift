...
final class AgentProvisionerTests: XCTestCase {
    func testInstallsOpenCodeProvider() throws {
        ...
        let settings = api2agentSettings(port: 8787)
        try provisioner.install(.opencode, settings: settings)

        let config = home.appending(path: ".config/opencode/opencode.json")
        let text = try String(contentsOf: config, encoding: .utf8)
        XCTAssertTrue(text.contains("api2agent"))
        XCTAssertTrue(text.contains(api2agentBrand.displayName))
        ...
        let cursorProvider = try XCTUnwrap(providers["api2agent"] as? [String: Any])
    ...
    func testInstallOpenCodeRemovesLegacyHostedProviders() throws {
        ...
        XCTAssertNotNil(providers["api2agent"])
        XCTAssertEqual(root["model"] as? String, "api2agent/composer-2.5-fast")
    }
    ...
    func testOpenCodeAndKiloRespectXDGConfigHome() throws {
        ...
        XCTAssertTrue(opencodeText.contains(api2agentBrand.displayName))
        XCTAssertTrue(kiloText.contains("api2agent"))
    ...
    func testInstallsCodexProvider() throws {
        ...
        let text = try String(contentsOf: config, encoding: .utf8)
        let profile = try String(contentsOf: home.appending(path: ".codex/api2agent.config.toml"), encoding: .utf8)
        let fastProfile = try String(contentsOf: home.appending(path: ".codex/api2agent-fast.config.toml"), encoding: .utf8)
        let catalog = try readJSONObject(home.appending(path: ".codex/api2agent.models.json"))
        XCTAssertTrue(text.contains("[model_providers.api2agent]"))
        XCTAssertTrue(text.contains("name = \"\(api2agentBrand.displayName)\""))
        XCTAssertTrue(text.contains("[model_providers.api2agent.auth]"))
        ...
        XCTAssertTrue(text.contains("model_provider = \"api2agent\""))
        XCTAssertTrue(text.contains("model = \"deepseek-v4-pro\""))
        XCTAssertTrue(text.contains("model_catalog_json = \"\(home.path)/.codex/api2agent.models.json\""))
        XCTAssertFalse(text.contains("[profiles.api2agent]"))
        XCTAssertFalse(text.contains("[profiles.api2agent-fast]"))
        XCTAssertTrue(profile.contains("model_provider = \"api2agent\""))
        XCTAssertTrue(profile.contains("model = \"deepseek-v4-pro\""))
        XCTAssertTrue(profile.contains("model_catalog_json = \"\(home.path)/.codex/api2agent.models.json\""))
        XCTAssertTrue(fastProfile.contains("model_provider = \"api2agent\""))
        XCTAssertTrue(fastProfile.contains("model = \"deepseek-v4-flash\""))
        XCTAssertTrue(fastProfile.contains("model_catalog_json = \"\(home.path)/.codex/api2agent.models.json\""))
        let models = try XCTUnwrap(catalog["models"] as? [[String: Any]])
        XCTAssertTrue(models.contains { $0["slug"] as? String == "deepseek-v4-pro" })
        XCTAssertTrue(models.contains { $0["slug"] as? String == "deepseek-v4-flash" })
    ...
    func testCodexInstallUpdatesExistingProviderWithoutDuplicatingSections() throws {
        ...
        let profile = try String(contentsOf: home.appending(path: ".codex/api2agent.config.toml"), encoding: .utf8)
        let fastProfile = try String(contentsOf: home.appending(path: ".codex/api2agent-fast.config.toml"), encoding: .utf8)
        XCTAssertEqual(countOccurrences(of: "[model_providers.api2agent]", in: text), 1)
        XCTAssertEqual(countOccurrences(of: "[model_providers.api2agent.auth]", in: text), 1)
        XCTAssertEqual(countOccurrences(of: "[profiles.api2agent]", in: text), 0)
        XCTAssertEqual(countOccurrences(of: "[profiles.api2agent-fast]", in: text), 0)
    ...
    func testInstallsClineAndKiloProfiles() throws {
        ...
        let kiloProvider = try XCTUnwrap(kiloProviders["api2agent"] as? [String: Any])
    ...
    func testInstallsRooCodeAutoImportProfile() throws {
        ...
        XCTAssertTrue(importText.contains("\"currentApiConfigName\" : \"\(api2agentBrand.displayName)\""))
        ...
        let primary = try XCTUnwrap(apiConfigs[api2agentBrand.displayName] as? [String: Any])
        let fast = try XCTUnwrap(apiConfigs["\(api2agentBrand.displayName) Fast"] as? [String: Any])
    ...
    func testInstallAllUpdatesStaleProviderWithoutDuplicatingInstalledConfigs() throws {
        let home = try temporaryHome()
        let provisioner = AgentProvisioner(homeDirectory: home)
        let original = api2agentSettings(port: 8787)
        let moved = api2agentSettings(port: 9999)
        ...
        XCTAssertEqual(countOccurrences(of: "\"api2agent\"", in: opencode), 1)
        XCTAssertEqual(countOccurrences(of: "[model_providers.api2agent]", in: codex), 1)
        XCTAssertEqual(countOccurrences(of: "\"api2agent\"", in: kilo), 1)
        XCTAssertEqual(countOccurrences(of: "\"api2agent\"", in: pi), 1)
    ...
    func testStatusesTreatHostedapi2agentConfigsAsRemoteProvidersNeedingLocalUpdate() throws {
        let home = try temporaryHome()
        let provisioner = AgentProvisioner(homeDirectory: home)
        let settings = api2agentSettings(port: 8787)
    ...
    private func temporaryHome() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appending(path: "api2agentTests-\(UUID().uuidString)")
