...
OLD_APP_DIR="$ROOT_DIR/dist/API for Cursor.app"
LEGACY_APP_DIR="$ROOT_DIR/dist/api2agent.app"
CONTENTS_DIR="$APP_DIR/Contents"
MACOS_DIR="$CONTENTS_DIR/MacOS"
FRAMEWORKS_DIR="$CONTENTS_DIR/Frameworks"
RESOURCES_DIR="$CONTENTS_DIR/Resources"
ICONSET_DIR="$RESOURCES_DIR/API2Agent.iconset"
APP_ICON_SOURCE="$ROOT_DIR/Sources/api2agent/Resources/API2Agent.png"
...
SWIFT_BUILD_FLAGS=()
if [ "${CURSOR_API_SWIFTPM_DISABLE_SANDBOX:-0}" = "1" ]; then
  SWIFT_BUILD_FLAGS+=(--disable-sandbox)
fi
mkdir -p "$CLANG_MODULE_CACHE_PATH"
swift build --package-path "$ROOT_DIR" -c release "${SWIFT_BUILD_FLAGS[@]}"
rm -rf "$APP_DIR" "$OLD_APP_DIR" "$LEGACY_APP_DIR"
mkdir -p "$MACOS_DIR" "$FRAMEWORKS_DIR" "$RESOURCES_DIR"
cp "$BUILD_DIR/api2agent" "$MACOS_DIR/$EXECUTABLE_NAME"
...
if [ -d "$BUILD_DIR/api2agent_api2agent.bundle" ]; then
  cp -R "$BUILD_DIR/api2agent_api2agent.bundle" "$RESOURCES_DIR/"
fi
[ -s "$APP_ICON_SOURCE" ] || { echo "Missing app icon source at $APP_ICON_SOURCE" >&2; exit 1; }
cp "$APP_ICON_SOURCE" "$RESOURCES_DIR/API2Agent.png"
...
if hasCompleteRouting {
    let outputURL = resourcesDirectory.appendingPathComponent("api2agentTransportDefaults.plist")
...
SWIFT
write_icns_from_png() {
  swift - "$APP_ICON_SOURCE" "$RESOURCES_DIR/API2Agent.icns" <<'SWIFT'
import Foundation

let sourceURL = URL(fileURLWithPath: CommandLine.arguments[1])
let outputURL = URL(fileURLWithPath: CommandLine.arguments[2])
let png = try Data(contentsOf: sourceURL)
let totalLength = 16 + png.count
guard totalLength <= Int(UInt32.max), png.count + 8 <= Int(UInt32.max) else {
    FileHandle.standardError.write(Data("App icon PNG is too large for ICNS fallback.\n".utf8))
    exit(1)
}
...
func appendUInt32BE(_ value: UInt32) {
    var bigEndian = value.bigEndian
    withUnsafeBytes(of: &bigEndian) { output.append(contentsOf: $0) }
}

appendASCII("icns")
appendUInt32BE(UInt32(totalLength))
appendASCII("ic10")
appendUInt32BE(UInt32(png.count + 8))
output.append(png)
...
func writeIcon(points: Int, scale: Int, name: String) throws {
    ...
    guard let context = CGContext(
        ...
        throw NSError(domain: "api2agentIcon", code: 1)
    }
    ...
    guard let image = context.makeImage() else {
        throw NSError(domain: "api2agentIcon", code: 2)
    }
    let representation = NSBitmapImageRep(cgImage: image)
    guard let data = representation.representation(using: .png, properties: [:]) else {
        throw NSError(domain: "api2agentIcon", code: 3)
    }
...
SWIFT
if ! iconutil -c icns "$ICONSET_DIR" -o "$RESOURCES_DIR/API2Agent.icns"; then
  echo "Warning: iconutil could not create API2Agent.icns from generated artwork; trying source artwork." >&2
  rm -f "$RESOURCES_DIR/API2Agent.icns"
fi
rm -rf "$ICONSET_DIR"
...
# but product builds should reflect the curated artwork in API2Agent.png.
mkdir -p "$ICONSET_DIR"
source_icon() {
  local pixels="$1"
  local output="$2"
...
if ! iconutil -c icns "$ICONSET_DIR" -o "$RESOURCES_DIR/API2Agent.icns"; then
  echo "Warning: iconutil could not create API2Agent.icns from source artwork; writing ICNS fallback." >&2
  if ! write_icns_from_png; then
    rm -f "$RESOURCES_DIR/API2Agent.icns"
    exit 1
  fi
fi
...
<dict>
  ...
  <string>com.standardagents.API2Agent</string>
...
rm -f "$ROOT_DIR/dist/API2Agent.zip" "$ROOT_DIR/dist/API for Cursor.zip" "$ROOT_DIR/dist/api2agent.zip"
ditto -c -k --keepParent "$APP_DIR" "$ROOT_DIR/dist/API2Agent.zip"
if [ "${CURSOR_API_SKIP_PACKAGE_VERIFY:-0}" != "1" ]; then
