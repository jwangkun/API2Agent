...
verify_deep_opencode_todo_app() {
  ...
  (
    cd "$deep_project"
    HOME="$temp_home" XDG_CONFIG_HOME="$temp_config" \
      opencode --pure run --agent build --model api2agent/composer-2.5-fast --format json --dangerously-skip-permissions \
...
JSON
  ...
  (
    HOME="$pi_home" PI_CODING_AGENT_DIR="$pi_agent_dir" \
      pi --provider api2agent --model composer-2.5-fast --no-session -p "Reply exactly: pismoke" >"$pi_output" 2>&1
  ) &
  pi_pid=$!
  ...
  (
    cd "$pi_tool_project"
    HOME="$pi_home" PI_CODING_AGENT_DIR="$pi_agent_dir" \
      pi --provider api2agent --model composer-2.5-fast --no-session -p \
...
TOML
  ...
  cat >"$codex_home/.codex/api2agent-fast.config.toml" <<'TOML'
...
TOML
  ...
  wait "$codex_pid" >/dev/null 2>&1 || true
  cat "$codex_output"
  grep -F "provider: api2agent" "$codex_output" >/dev/null || fail "Codex did not use the api2agent provider"
...
JSON
  ...
  models_output="$(cd "$temp_project" && HOME="$temp_home" XDG_CONFIG_HOME="$temp_config" opencode --pure models api2agent 2>&1)"
  printf '%s\n' "$models_output"
  grep -F "api2agent/composer-2.5-fast" <<<"$models_output" >/dev/null || fail "OpenCode did not list composer-2.5-fast"
  ...
  tmux new-session -d -x 180 -y 56 -s "$session" "cd \"$temp_project\" && HOME=\"$temp_home\" XDG_CONFIG_HOME=\"$temp_config\" opencode --pure run --interactive --dangerously-skip-permissions --model api2agent/composer-2.5-fast"
  sleep 4
