...
printf '%s\n' "$models_output"
grep -F "api2agent/composer-2.5" <<<"$models_output" >/dev/null || fail "OpenCode did not list composer-2.5"
...
if [ "$status" = "needs_unlock" ]; then
  ...
  (
    cd "$temp_project"
    HOME="$temp_home" XDG_CONFIG_HOME="$temp_config" opencode --pure run --model api2agent/composer-2.5 "say hello" >"$run_output" 2>&1
  ) &
  run_pid=$!
