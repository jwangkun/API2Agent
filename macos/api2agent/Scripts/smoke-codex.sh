...
cat > "$temp_home/.codex/api2agent.config.toml" <<TOML
model_provider = "api2agent"
model = "composer-2.5"
TOML

cat > "$temp_home/.codex/api2agent-fast.config.toml" <<TOML
model_provider = "api2agent"
...
grep -F "provider: api2agent" "$run_output" >/dev/null || fail "Codex did not use the api2agent provider"
