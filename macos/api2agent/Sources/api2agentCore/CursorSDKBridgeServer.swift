...
actor CursorSDKBridgeServer {
    ...
    func endpoint(settings: api2agentSettings) async throws -> CursorSDKBridgeEndpoint {
    ...
    private func start(script: URL, settings: api2agentSettings) async throws -> UInt16 {
        ...
        for port in 8792...8892 {
            ...
            do {
                ...
                stop()
                lastError = api2agentError.transport("Cursor SDK bridge did not become ready.")
            } catch {
                stop()
                lastError = error
            }
        }
        throw lastError ?? api2agentError.transport("Could not start Cursor SDK bridge.")
    }

    private func launch(script: URL, port: UInt16, settings: api2agentSettings) throws {
        let process = Process()
    ...
    private func runtimeExecutable() throws -> URL {
        ...
        throw api2agentError.invalidConfiguration("\(api2agentBrand.displayName) is missing its bundled SDK bridge runtime. Repackage the app with Bun or Node bundled.")
    }
    ...
    private func systemExecutable(named command: String) throws -> URL? {
        ...
        guard !path.isEmpty, FileManager.default.isExecutableFile(atPath: path) else {
            throw api2agentError.invalidConfiguration("\(command) is installed but could not be used for the SDK bridge.")
        }
    ...
    private func tcpPortIsOpen(_ port: UInt16) async -> Bool {
        await withCheckedContinuation { continuation in
            ...
            let queue = DispatchQueue(label: "api2agent.SDKBridgePortCheck.\(port)")
    ...
    private func bridgeScriptURL() throws -> URL {
        ...
        throw api2agentError.invalidConfiguration("Cursor SDK bridge script is missing. Repackage \(api2agentBrand.displayName) or run from the repository checkout.")
    }
}
