...
public protocol CursorSDKHarness: Sendable {
    func validate(settings: api2agentSettings, authorization: String?) throws
...
public extension CursorSDKHarness {
    ...
    func complete(prepared: PreparedChatRequest, settings: api2agentSettings, authorization: String?) async throws -> CursorSDKOutput {
        var text = ""
...
public struct LocalCursorSDKHarness: CursorSDKHarness {
    ...
    public static func warmUpBridge(settings: api2agentSettings) {
        Task {
    ...
    public func validate(settings: api2agentSettings, authorization: String?) throws {
        let apiKey = try Self.resolvedapi2agentKeyForRequest(from: authorization, settings: settings)
        guard !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw api2agentError.unauthorized
        }
    }

    public func stream(prepared: PreparedChatRequest, settings: api2agentSettings, authorization: String?) -> AsyncThrowingStream<CursorSDKStreamEvent, any Error> {
    ...
    private func runSDKBridgeRequestWithTransportRetry(
        ...
        throw lastError ?? api2agentError.transport("Cursor SDK bridge request failed.")
    }
    ...
    private func runActualSDKBridgeRequest(
        ...
        do {
            ...
            throw api2agentError.transport("Cursor SDK bridge request failed: \(error.localizedDescription)")
        }
        guard let http = response as? HTTPURLResponse else {
            throw api2agentError.transport("Cursor SDK bridge did not return an HTTP response.")
        }
        guard (200..<300).contains(http.statusCode) else {
            let data = try await collectData(bytes)
            let text = String(data: data, encoding: .utf8) ?? "status \(http.statusCode)"
            throw http.statusCode == 401 ? api2agentError.unauthorized : api2agentError.upstream(text)
        }
        var emittedText = ""
        var emittedToolCalls: [CursorToolCall] = []
        var finalOutput: CursorSDKOutput?
        ...
        guard let finalOutput else {
            throw api2agentError.transport("Cursor SDK bridge stream ended without a final output.")
        }
    ...
    private func handleBridgeEventLine(
        ...
        guard let object = try JSONSerialization.jsonObject(with: line) as? [String: Any],
            ...
            throw api2agentError.transport("Cursor SDK bridge returned an invalid stream event.")
        }
        ...
        case "done":
            guard let output = bridgeOutput(from: object["output"], agentID: agentID, runID: runID) else {
                throw api2agentError.transport("Cursor SDK bridge returned an invalid final output.")
            }
            finalOutput = output
        case "error":
    ...
    static func bridgeStreamError(from error: [String: Any]?) -> api2agentError {
    ...
    private func isRetryableBridgeTransportError(_ error: any Error) -> Bool {
        if case .transport = error as? api2agentError {
            return true
        }
        return false
    }
    ...
    static func resolvedapi2agentKey(from authorization: String?, settings: api2agentSettings) -> String {
        guard let token = bearerToken(authorization), !isLocalPlaceholderToken(token) else {
            return settings.api2agentKey
        }
        return token
    }

    static func resolvedapi2agentKeyForRequest(from authorization: String?, settings: api2agentSettings) throws -> String {
        ...
        if settings.keychainapi2agentKeyAvailable {
            throw api2agentError.keychainLocked
        }
        return ""
    }
...
private actor CursorSDKBridgeLifecycle {
    ...
    func warmUp(settings: api2agentSettings) {
        warmUpTask?.cancel()
