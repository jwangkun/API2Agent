...
public final class AgentProvisioner: @unchecked Sendable {
    ...
    private static let rooProfileName = api2agentBrand.displayName
    private static let rooFastProfileName = "\(api2agentBrand.displayName) Fast"
    ...
    public func statuses(settings: api2agentSettings) -> [AgentIntegrationStatus] {
        Self.visibleIntegrationIDs.map { status(for: $0, settings: settings) }
    }

    public func status(for id: AgentIntegrationID, settings: api2agentSettings) -> AgentIntegrationStatus {
        switch id {
        case .opencode:
    ...
    public func install(_ id: AgentIntegrationID, settings: api2agentSettings) throws {
        switch id {
        case .opencode:
    ...
    public func installAll(settings: api2agentSettings) throws -> [AgentIntegrationStatus] {
    ...
    private func opencodeStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func installOpenCode(settings: api2agentSettings) throws {
        let url = opencodeConfigURL()
        ...
        provider["api2agent"] = [
            ...
            "name": api2agentBrand.displayName,
            "options": [
        ...
        if let model = stringValue(root["model"]),
           model.hasPrefix("cursor/") || model.hasPrefix("cursorsdk/") {
            root["model"] = "api2agent/composer-2.5-fast"
    ...
    private func codexStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func installCodex(settings: api2agentSettings) throws {
        let url = codexConfigURL()
        try ensureParentDirectory(url)
        var text = (try? String(contentsOf: url, encoding: .utf8)) ?? ""
        let providerBaseURL = tomlSectionValue(section: "model_providers.api2agent", key: "base_url", in: text)
        ...
        [model_providers.api2agent.auth]
        command = "/bin/echo"
        args = ["cursor-local"]
        refresh_interval_ms = 300000
        """
        text = replaceTOMLBlock(named: "model_providers.api2agent.auth", in: text, replacement: "")
        text = replaceTOMLBlock(named: "model_providers.api2agent", in: text, replacement: "")
        text = replaceTOMLBlock(named: "profiles.api2agent", in: text, replacement: "")
        text = replaceTOMLBlock(named: "profiles.api2agent-fast", in: text, replacement: "")
        ...
        try writeCodexProfile(name: "api2agent", model: "deepseek-v4-pro")
        try writeCodexProfile(name: "api2agent-fast", model: "deepseek-v4-flash")
        configureActiveCodexModel()
    }

    private func configureActiveCodexModel() {
        let url = codexConfigURL()
        guard var text = try? String(contentsOf: url, encoding: .utf8) else { return }
        text = upsertTopLevelTOMLString(key: "model_provider", value: "api2agent", in: text)
        text = upsertTopLevelTOMLString(key: "model", value: "deepseek-v4-pro", in: text)
        text = upsertTopLevelTOMLString(key: "model_catalog_json", value: codexModelCatalogURL().path, in: text)
        try? writeText(text, to: url)
    }
    ...
    private func writeCodexProfile(name: String, model: String) throws {
        ...
        model = "\(model)"
        model_catalog_json = \(tomlQuotedValue(codexModelCatalogURL().path))
        """
        try writeText(text + "\n", to: codexProfileConfigURL(name))
    }

    private func writeCodexModelCatalog() throws {
        let models = ComposerModels.all.map(OpenAICompatibility.codexModelObject)
        try writeJSONObject(["models": models], to: codexModelCatalogURL())
    }

    private func vscodeStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func installVSCode(settings: api2agentSettings) throws {
        let url = vscodeLanguageModelsURL()
        ...
        array.removeAll { item in
            ...
            return name == "api2agent" || name == api2agentBrand.displayName
        }
        array.append([
            "name": api2agentBrand.displayName,
    ...
    private func extensionStatus(id: AgentIntegrationID, settings: api2agentSettings) -> AgentIntegrationStatus {
        if id == .cline {
    ...
    private func piStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
        let url = piModelsURL()
    ...
    private func continueStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func aiderStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func rooStatus(settings: api2agentSettings) -> AgentIntegrationStatus {
    ...
    private func codexConfigMatches(_ text: String, settings: api2agentSettings) -> Bool {
        topLevelTOMLValue("model_provider", in: text) == "api2agent"
            && topLevelTOMLValue("model", in: text) == "deepseek-v4-pro"
            && topLevelTOMLValue("model_catalog_json", in: text) == codexModelCatalogURL().path
            && text.contains("[model_providers.api2agent]")
            && text.contains("name = \"\(api2agentBrand.displayName)\"")
            && tomlSectionValue(section: "model_providers.api2agent", key: "base_url", in: text) != nil
            && text.contains("wire_api = \"responses\"")
            && text.contains("[model_providers.api2agent.auth]")
            && text.contains("command = \"/bin/echo\"")
            && text.contains("args = [\"cursor-local\"]")
            && codexModelCatalogMatches()
            && codexProfileConfigMatches(name: "api2agent", model: "deepseek-v4-pro")
            && codexProfileConfigMatches(name: "api2agent-fast", model: "deepseek-v4-flash")
    }

    private func codexProfileConfigMatches(name: String, model: String) -> Bool {
        let text = fileText(codexProfileConfigURL(name))
        return topLevelTOMLValue("model_provider", in: text) == "api2agent"
            && topLevelTOMLValue("model", in: text) == model
            && topLevelTOMLValue("model_catalog_json", in: text) == codexModelCatalogURL().path
    }

    private func codexModelCatalogMatches() -> Bool {
        guard fileManager.fileExists(atPath: codexModelCatalogURL().path),
              let root = try? readJSONObject(codexModelCatalogURL(), defaultValue: [:]),
              let models = root["models"] as? [[String: Any]] else {
            return false
        }
        return ComposerModels.all.allSatisfy { model in
            models.contains { stringValue($0["slug"]) == model.id }
        }
    }

    private func vscodeConfigMatches(_ url: URL, settings: api2agentSettings) -> Bool {
        ...
        return array.contains { item in
            guard let record = item as? [String: Any],
                  stringValue(record["name"]) == api2agentBrand.displayName,
    ...
    private func opencodeConfigMatches(settings: api2agentSettings) -> Bool {
        let url = opencodeConfigURL()
        guard fileManager.fileExists(atPath: url.path),
              let root = try? readJSONObject(url, defaultValue: [:]),
              let providers = root["provider"] as? [String: Any],
              let provider = providers["api2agent"] as? [String: Any] else {
            return false
        }
        return localProviderMatches(provider, settings: settings)
    }

    private func clineConfigMatches(settings: api2agentSettings) -> Bool {
    ...
    private func kiloConfigMatches(settings: api2agentSettings) -> Bool {
        let url = kiloConfigURL()
        guard fileManager.fileExists(atPath: url.path),
              let root = try? readJSONObject(url, defaultValue: [:]),
              let providers = root["provider"] as? [String: Any],
    ...
    private func piConfigMatches(settings: api2agentSettings) -> Bool {
        let url = piModelsURL()
        guard fileManager.fileExists(atPath: url.path),
              let root = try? readJSONObject(url, defaultValue: [:]),
              let providers = root["providers"] as? [String: Any],
              let provider = providers["api2agent"] as? [String: Any],
    ...
    private func continueConfigMatches(_ text: String, settings: api2agentSettings) -> Bool {
    ...
    private func aiderConfigMatches(_ text: String, settings: api2agentSettings) -> Bool {
    ...
    private func aiderModelMetadataMatches(settings: api2agentSettings) -> Bool {
    ...
    private func aiderModelSettingsMatches(settings: api2agentSettings) -> Bool {
    ...
    private func rooImportSettingsMatches(settings: api2agentSettings) -> Bool {
    ...
    private func rooProfileMatches(_ profile: [String: Any], model: ComposerModel, id: String, settings: api2agentSettings) -> Bool {
    ...
    private func installCline(settings: api2agentSettings) throws {
        let globalStateURL = clineGlobalStateURL()
    ...
    private func installKilo(settings: api2agentSettings) throws {
        let url = kiloConfigURL()
        ...
        root["provider"] = provider
        if root["model"] == nil {
            root["model"] = "api2agent/composer-2.5"
        }
        try writeJSONObject(root, to: url)
    }

    private func installPi(settings: api2agentSettings) throws {
        let url = piModelsURL()
    ...
    private func installContinue(settings: api2agentSettings) throws {
        let url = continueConfigURL()
    ...
    private func installAider(settings: api2agentSettings) throws {
    ...
    private func installRoo(settings: api2agentSettings) throws {
    ...
    private func codexModelCatalogURL() -> URL {
        homeDirectory.appending(path: ".codex/api2agent.models.json")
    }
    ...
    private func continueModelsBlock(settings: api2agentSettings) -> String {
        var lines = [
    ...
    private func aiderConfigBlock(settings: api2agentSettings) -> String {
        [
    ...
    private func rooImportSettings(settings: api2agentSettings) -> [String: Any] {
        let primaryModel = ComposerModels.all[0]
        let fastModel = ComposerModels.all[1]
        return [
            "providerProfiles": [
    ...
    private func rooProfile(for model: ComposerModel, id: String, settings: api2agentSettings) -> [String: Any] {
        [
            "id": id,
            "apiProvider": "openai",
    ...
    private func topLevelTOMLValue(_ key: String, in text: String) -> String? {
        for line in text.split(separator: "\n", omittingEmptySubsequences: false).map(String.init) {
            let trimmed = line.trimmingCharacters(in: .whitespaces)
            ...
            let currentKey = String(trimmed[..<equals]).trimmingCharacters(in: .whitespaces)
            guard currentKey == key else { continue }
            let rawValue = String(trimmed[trimmed.index(after: equals)...])
                .trimmingCharacters(in: .whitespacesAndNewlines)
            return unquotedTOMLString(rawValue)
        }
        return nil
    }

    private func tomlSectionValue(section: String, key: String, in text: String) -> String? {
        var inSection = false
        for line in text.split(separator: "\n", omittingEmptySubsequences: false).map(String.init) {
            let trimmed = line.trimmingCharacters(in: .whitespaces)
            guard !trimmed.hasPrefix("#") else { continue }
    ...
    private func upsertTopLevelTOMLString(key: String, value: String, in text: String) -> String {
        var lines = text.split(separator: "\n", omittingEmptySubsequences: false).map(String.init)
        let firstSectionIndex = lines.firstIndex { line in
            line.trimmingCharacters(in: .whitespaces).hasPrefix("[")
        } ?? lines.count
        let replacement = "\(key) = \(tomlQuotedValue(value))"
        for index in 0..<firstSectionIndex {
        ...
        lines.insert(replacement, at: firstSectionIndex)
        return lines.joined(separator: "\n")
    }
    ...
    private func tomlQuotedValue(_ value: String) -> String {
        let escaped = value
            .replacingOccurrences(of: "\\", with: "\\\\")
            .replacingOccurrences(of: "\"", with: "\\\"")
        return "\"\(escaped)\""
    ...
    private func localProviderMatches(_ provider: [String: Any], settings: api2agentSettings) -> Bool {
    ...
    private func providerStatusDetail(text: String, settings: api2agentSettings) -> String {
        if text.contains(settings.baseURL.absoluteString) {
            return "Provider needs update"
        }
        if containsHostedapi2agentURL(text) {
        ...
        if text.contains("api2agent") || text.contains("api-for-cursor") || text.contains(api2agentBrand.displayName) || text.contains("api2agent") {
            return "Provider found with a different local URL"
        }
        return "Ready to install"
    }

    private func continueProviderStatusDetail(text: String, settings: api2agentSettings) -> String {
    ...
    private func containsHostedapi2agentURL(_ text: String) -> Bool {
    ...
    private func readJSONObject(_ url: URL, defaultValue: [String: Any]) throws -> [String: Any] {
        ...
        guard let object = value as? [String: Any] else {
            throw api2agentError.badRequest("\(url.lastPathComponent) must contain a JSON object.")
        }
        return object
    }
    ...
    private func readJSONArray(_ url: URL, defaultValue: [Any]) throws -> [Any] {
        ...
        guard let array = value as? [Any] else {
            throw api2agentError.badRequest("\(url.lastPathComponent) must contain a JSON array.")
        }
        return array
    }
    ...
    private func readJSONValue(_ url: URL) throws -> Any {
        ...
        do {
            ...
            throw api2agentError.badRequest("Could not parse \(url.lastPathComponent): \(error.localizedDescription)")
        }
    }
