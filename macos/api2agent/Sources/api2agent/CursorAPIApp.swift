...
@main
@MainActor
final class api2agentAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    private static var retainedDelegate: api2agentAppDelegate?

    private let model = api2agentAppModel()
    ...
    static func main() {
        ...
        let delegate = api2agentAppDelegate()
        ...
        DispatchQueue.main.async {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                delegate.model.refreshIntegrations()
                delegate.model.installAllIntegrations()
            }
        }
        app.run()
    }
    ...
    private func installStatusItemIfNeeded() {
        ...
        if let button = item.button {
            ...
            button.toolTip = api2agentBrand.displayName
        }
        statusItem = item
    }
    ...
    private static func standardAgentsMenuBarBitmapIcon() -> NSImage? {
        ...
        image.accessibilityDescription = api2agentBrand.displayName
        return image
    }
    ...
    private func updateStatusMenu() {
        guard let statusItem else { return }
        let menu = NSMenu(title: api2agentBrand.displayName)
        ...
        menu.addItem(.separator())
        let quitItem = NSMenuItem(title: "Quit \(api2agentBrand.displayName)", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    private func revealMainWindow() {
        ...
        if let window = mainWindow {
            window.contentViewController = NSHostingController(rootView: api2agentAppRootView(model: model))
            show(window)
            return
        }
        ...
        window.title = api2agentBrand.displayName
    ...
    @objc private func checkForUpdates(_ sender: Any?) {
        guard shouldOfferUpdates else { return }
        updaterController.startUpdater()
        updaterController.checkForUpdates(sender)
    }

    private var shouldOfferUpdates: Bool {
    ...
    private func installMainMenu() {
        ...
        let appMenu = NSMenu(title: api2agentBrand.displayName)
        appMenu.addItem(withTitle: "About \(api2agentBrand.displayName)", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        let updatesItem = appMenu.addItem(withTitle: "Check for Updates...", action: #selector(checkForUpdates(_:)), keyEquivalent: "")
        updatesItem.target = self
        updatesItem.isEnabled = shouldOfferUpdates
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Hide \(api2agentBrand.displayName)", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        ...
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Quit \(api2agentBrand.displayName)", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        ...
        let showWindowItem = windowMenu.addItem(withTitle: "Show \(api2agentBrand.displayName)", action: #selector(showMainWindow(_:)), keyEquivalent: "0")
...
private struct api2agentAppRootView: View {
