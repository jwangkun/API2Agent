...
@MainActor
final class api2agentAppModel: ObservableObject {
    private static let portFallbackLimit = 20
    private static let recentRequestLimit = 6

    @Published var settings: api2agentSettings
    ...
    private lazy var server = LocalAPIServer(settingsProvider: { [weak self] in
        DispatchQueue.main.sync {
            self?.settings ?? api2agentSettings()
        }
    ...
    var hasapi2agentKey: Bool {
        settings.hasapi2agentKey
    }
    ...
    var canCheckSDK: Bool {
        hasapi2agentKey && sdkConfigured && !isCheckingSDK
    }
    ...
    var sdkCheckNeedsAPIKeyAction: Bool {
        ...
        return message == (api2agentError.unauthorized.errorDescription ?? "")
    ...
    func startServer(allowKeychainPrompt: Bool = true, resolveSavedKey: Bool = true) {
        do {
            if resolveSavedKey && settings.hasInlineapi2agentKey {
                settings = try store.resolvingapi2agentKey(in: settings, allowUserPrompt: allowKeychainPrompt)
                settings.keychainapi2agentKeyAvailable = true
                needsKeychainPermission = false
            } else {
                needsKeychainPermission = settings.keychainapi2agentKeyAvailable && !settings.hasInlineapi2agentKey
            }
            ...
            if activePort != requestedPort {
                statusText = "Port \(requestedPort) was busy; listening on \(activePort)"
                refreshIntegrations()
            } else {
            ...
            statusText = "Click Start to allow \(api2agentBrand.displayName) to read the saved key from Keychain"
            lastError = nil
        } catch AppSettingsStoreError.missingapi2agentKey {
            isRunning = false
    ...
    func restartServer() {
        ...
        let shouldResolveSavedKey = settings.hasInlineapi2agentKey && !needsKeychainPermission
        stopServer()
    ...
    func saveSettings() {
        ...
        refreshIntegrations()
        if !hasapi2agentKey || !sdkConfigured {
            stopServer()
        } else if isRunning {
            restartServer()
        } else {
    ...
    func checkSDKConnectivity() {
        ...
        Task {
            do {
                let resolved = try store.resolvingapi2agentKey(in: settings, allowUserPrompt: true)
                settings = resolved
                settings.keychainapi2agentKeyAvailable = true
                needsKeychainPermission = false
