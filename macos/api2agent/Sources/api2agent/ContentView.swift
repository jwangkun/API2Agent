...
struct ContentView: View {
    @ObservedObject var model: api2agentAppModel
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                if let lastError = model.lastError {
                    AppNoticeBanner(
                        title: model.statusText == "Could not start" ? "Could not start" : "\(api2agentBrand.displayName) needs attention",
                        message: lastError,
        ...
        .onChange(of: model.settings.api2agentKey) { _, _ in
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            APIKeyHeaderButton(
                ...
                needsAttention: !model.hasapi2agentKey || model.needsKeychainPermission || model.sdkCheckFailed
            ) {
    ...
    private var statusTone: StatusPill.Tone {
        model.hasapi2agentKey && !model.needsKeychainPermission && model.sdkConfigured ? .ok : .warning
    }
}
    ...
    var body: some View {
        GlassPanel {
            VStack(alignment: .leading, spacing: 18) {
                ...
                Text("The local API starts after the key is saved. The key is stored in macOS Keychain and is only read by \(api2agentBrand.displayName).")
                    .foregroundStyle(.secondary)
                ...
                HStack(spacing: 10) {
                    ...
                    PillActionButton("Save Key") {
                        model.settings.api2agentKey = trimmedDraftKey
...
struct APIKeyDialog: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text(model.hasapi2agentKey ? "Enter a new Cursor API key to replace the saved key." : "Enter a Cursor API key to start the local API.")
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text("Saved locally in macOS Keychain and used only by \(api2agentBrand.displayName).")
                .font(.caption)
                .foregroundStyle(.secondary)
            ...
            HStack {
                ...
                PillActionButton(model.hasapi2agentKey ? "Replace Key" : "Save Key") {
                    save()
                }
                .disabled(!canSaveKey)
            }
        ...
        HStack(alignment: .center, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("Saved key needs permission")
                    .font(.callout.weight(.semibold))
                Text("\(api2agentBrand.displayName) stores your Cursor API key in macOS Keychain and reads it only when you unlock Composer access.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            }
            Spacer()
...
struct ConnectionPage: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if model.hasapi2agentKey && !model.needsKeychainPermission {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                SettingsSummaryTile(
                    icon: "key.fill",
                    title: model.hasapi2agentKey ? "Key Saved" : "Key Required",
                    detail: "macOS Keychain",
                    tone: model.hasapi2agentKey ? .ok : .warning
                )
                SettingsSummaryTile(
                    icon: model.isRunning ? (model.needsKeychainPermission ? "lock.circle.fill" : "checkmark.circle.fill") : "power.circle",
                    title: model.isRunning ? (model.needsKeychainPermission ? "API Locked" : "API Running") : (model.sdkConfigured ? (model.hasapi2agentKey ? "Ready to Start" : "Needs Key") : "SDK Bridge Missing"),
            ...
            SettingsGroup(title: "Local Server", icon: "server.rack") {
                ...
                SettingsFieldRow(title: "Launch at Login", subtitle: "Start \(api2agentBrand.displayName) when macOS signs in") {
...
struct APIKeySettingsControl: View {
    ...
    var body: some View {
        Group {
            if model.hasapi2agentKey && !isReplacing {
                HStack(spacing: 10) {
                    Image(systemName: "checkmark.circle.fill")
                        .foregroundStyle(.secondary)
                    VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 8) {
                    SecureField("crsr_...", text: $draftKey)
                        .textFieldStyle(.roundedBorder)
                        .focused($keyFieldFocused)
                    PillActionButton(model.hasapi2agentKey ? "Save Key" : "Add Key") {
                        model.settings.api2agentKey = trimmedDraftKey
                        model.saveKeyStartAndCheckIfReady()
                        draftKey = ""
                        isReplacing = false
    ...
    var body: some View {
        DisclosureGroup(isExpanded: $isExpanded) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("\(api2agentBrand.displayName) runs a bundled local SDK bridge. These settings are only for diagnostics and compatibility labels; the app supplies the SDK bridge automatically.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .padding(.top, 6)
