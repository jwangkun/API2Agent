...
export async function saveSignup(env: Env, api2agentKey: string, me: CursorMe, input: { joinWaitlist: boolean }): Promise<SignupRecord> {
  ...
  const accountId = await accountIdForCursor(cursorUserId, cursorEmail, await sha256Hex(api2agentKey));
  const encrypted = await encryptText(api2agentKey, secret);
  const hint = api2agentKey.slice(-4);
...
export async function authenticateProxyKey(env: Env, proxyApiKey: string): Promise<AuthenticatedAccount | null> {
  ...
  const api2agentKey = await decryptText(
    account.cursor_api_key_ciphertext,
    account.cursor_api_key_iv,
  ...
  return { account, apiKey, api2agentKey };
}
