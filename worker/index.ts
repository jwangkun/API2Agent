import { collectCursorOutput, createCursorCompletion, resolveCursorModel, streamCursorText, verifyapi2agentKey } from "./cursor";
 ...
 */
type AuthResult =
  | { mode: "proxy"; accountId: string; api2agentKey: string }
  | { mode: "direct"; api2agentKey: string };
...
async function handleNotaryWebhook(request: Request, env: Env, url: URL, deps: Deps): Promise<Response> {
  ...
  const repository = env.GITHUB_RELEASE_REPOSITORY || "jwangkun/API2Agent";
...
async function handleSignup(request: Request, env: Env, ctx: ExecutionContext, deps: Deps): Promise<Response> {
  ...
  const api2agentKey = typeof body.api2agentKey === "string" ? body.api2agentKey.trim() : "";
  if (!api2agentKey) throw new HttpError("Cursor API key is required", 400, "invalid_request_error", "api2agentKey");

  const me = await verifyapi2agentKey(env, deps, api2agentKey);
  const name = typeof body.name === "string" ? body.name.trim() : "";
  const email = typeof body.email === "string" ? body.email.trim() : me.userEmail || "";
  const joinWaitlist = body.joinWaitlist === true;
  const signup = await saveSignup(env, api2agentKey, me, { joinWaitlist });
  if (joinWaitlist) {
    ctx.waitUntil(
  ...
  return json({
    account: {
      ...
      api2agentKeyHint: signup.account.cursor_api_key_hint
    },
...
async function handleOpenAiRoute(
  ...
  // Proxy DeepSeek models directly to DeepSeek API
  ...
  try {
    ...
    const completion = await createCursorCompletion(env, deps, auth.api2agentKey, {
...
async function handleSdkPreparedOpenAiRoute(input: {
  ...
  const completion = await createCursorSdkCompletion(input.env, input.deps, input.auth.api2agentKey, {
...
async function handleOpenCodeSdkChatRoute(
  ...
  try {
    const completion = await createCursorSdkCompletion(env, deps, auth.api2agentKey, {
      prompt: prepared.prompt,
      model: prepared.cursorModel,
...
async function handleResponseStateRoute(request: Request, auth: AuthResult, route: OpenAiRoute): Promise<Response> {
  ...
  if (route.kind === "responseCancel") {
    if (request.method !== "POST") return notFound();
    throw new HttpError("Only background responses can be cancelled. API2Agent runs responses synchronously.", 400, "invalid_request_error");
  }
...
async function responseOwnerKey(auth: AuthResult): Promise<string> {
  ...
  return `direct:${(await sha256Hex(auth.api2agentKey)).slice(0, 24)}`;
}
...
async function authenticate(request: Request, env: Env, route: OpenAiRoute): Promise<AuthResult | null> {
  ...
  if (token.startsWith("cmp_")) {
    ...
    return { mode: "proxy", accountId: auth.account.id, api2agentKey: auth.api2agentKey };
  }
  ...
  return { mode: "direct", api2agentKey: token };
}
