...
export async function verifyapi2agentKey(env: Env, deps: Deps, apiKey: string): Promise<CursorMe> {
...
export async function createCursorCompletion(
  ...
  const accessToken = await exchangeapi2agentKey(env, deps, apiKey);
...
export async function exchangeapi2agentKey(env: Env, deps: Deps, apiKey: string): Promise<string> {
...
async function getCursorAccountIdentity(env: Env, deps: Deps, apiKey: string): Promise<string> {
  ...
  const me = await verifyapi2agentKey(env, deps, apiKey);
  const identity =
